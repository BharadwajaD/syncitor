defmodule Syncitor.GroupRegistryTest do
  ...
  setup_all do
    supervisor_pid = start_supervised!({Syncitor.Supervisor, :init_arg})
  ...
  test "testing group creating in registry" do
    ...
    {:ok, gs_pid} = Syncitor.GroupRegistry.get_group_server("test_group_1")
  ...
  test "group registry should not crash even if a group server crashes" do
    ...
    {:ok, gs_pid} = Syncitor.GroupRegistry.get_group_server("test_crash_group")
    GenServer.stop(gs_pid, :bad_input)
    :timer.sleep(100) #give some time for the group_server to update its pid in registry
    {:ok, ngs_pid } = Syncitor.GroupRegistry.get_group_server("test_crash_group")
